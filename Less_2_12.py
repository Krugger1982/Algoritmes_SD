...
class SimpleGraph:
    ...
    def WeakVertices(self):
        ...
        for Vertexes in self.vertex:
            if Vertexes is not None:
                Vertexes.InTreangle = False                 # В начале поиска помечаем все существующие вершины графа как не входящие в треугольник
        result = []
        for current1 in range(len(self.vertex)):            # пробегаем по списку верщин
            if self.vertex[current1] is not None and not self.vertex[current1].InTreangle:
                # Если текущая вершина существует и еще не входит в треугольник
                for current2 in range (len(self.vertex)):   # Пробегаем по строчке матрицы смежности
                    if (self.m_adjacency[current1][current2] == 1 and current1 != current2):
                        # Если вершина current2 смежная c current1 и не является самой вершиной current1
                        for current3 in range (len(self.vertex)):   # смотрим в смежных вершинах current3
                            if self.m_adjacency[current2][current3] == 1 and current2 != current3 and self.m_adjacency[current1][current3] == 1:
                                # Если для вершины на смежной с current2 выполняется также условие смежности с current1 (и все они не совпадают друг с другом)
                                # тогда можно сделать вывод, что вершины current2, current2, и current2 образуют треугольник
                                self.vertex[current1].InTreangle = True     # помечаем их все как "члены треугольника
                                self.vertex[current2].InTreangle = True
                                self.vertex[current3].InTreangle = True
            if self.vertex[current1] is not None and not self.vertex[current1].InTreangle:
                # Если текущая вершина существует и после проверки всех ее соседей не помечена как "член треугольника"
                result.append(self.vertex[current1])
        return result
